                                ...
                                <div class="bg-white rounded-lg shadow-md p-6 mb-3">
                                    ...
                                    <div class="space-y-2">
                                        <label v-for="category in availableFilters.categories" :key="category.slug" class="flex items-center space-x-3 px-4 py-2 bg-blue-50 text-blue-700 rounded-md transition-all duration-200 ease-in-out cursor-pointer hover:bg-blue-100">
                                            <input type="checkbox" :disabled="postLoading" class="form-checkbox h-5 w-5 text-blue-600 rounded-sm border-gray-300 focus:ring-blue-500" :value="category.slug" v-model="filters.categories">
                                            <span class="text-base font-medium">{{ category.name }}</span>
                                        </label>
                                    </div>
                                </div>
                                <div class="flex space-x-3">
                                    <button type="reset" @click="resetFilters" :disabled="postLoading" class="w-full bg-[darksalmon] text-red font-medium py-2 px-4 rounded-lg hover:bg-opacity-50 transition">
                                        Reset
                                    </button>
                                    <button type="submit" :disabled="postLoading" class="w-full bg-blue-600 text-white font-medium py-2 px-4 rounded-lg hover:bg-blue-700 transition">
                                        Apply Filters
                                    </button>
                                </div>
                            </form>
