        ...
        <div class="space-y-6">
            ...
            <!-- Wishlist Items -->
                ...
                <div
                    ...
                    :key="item.id"
                                    class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-2xl transition-all duration-300 group relative border border-gray-100 hover:border-gold-200 hover:ring-2 hover:ring-gold-100 max-w-64 mx-auto w-full"
                >
                            <RouterLink :to="`/product/${item.slug}`" class="block relative">
                    ...
                    <button
                        :aria-label="$t('wishlist.remove_item_aria', { name: item.name })"
                    ...
                    >
                        <font-awesome-icon icon="times" class="text-red-500" />
                                ...
                                <div class="p-4">
                                    <h3 class="font-semibold text-sm md:text-base mb-1 line-clamp-1" :title="item.name">{{ item.name }}</h3>
                                    <div class="flex items-center justify-between text-[11px] md:text-xs text-gray-500 mb-2">
                                        <span v-if="item.sku" class="truncate">#{{ item.sku }}</span>
                                        <span v-if="(item.stock_quantity ?? 0) > 0" class="inline-flex items-center gap-1 rounded-full bg-green-50 text-green-700 px-2 py-0.5">
                                            <span class="w-1.5 h-1.5 bg-green-500 rounded-full"></span>
                                            {{ $t('shop.in_stock') }}
                                        </span>
                                    </div>

                                    <p class="text-gray-500 text-xs md:text-sm mb-2 line-clamp-2" v-if="item.short_description">
